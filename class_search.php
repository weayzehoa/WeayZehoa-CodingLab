    ...
    <div class="row">
        <div class="col-12">
            ...
            <?php 
                if($num >= 1){
                    do { ?>
                        <div>今日　<span class="text-primary"><?=$today?></span>　課程：<span class="text-primary"><?=$row['c_n_name']?></span>　時數： <span class="text-primary"><?=$row['c_num']?></span> 小時　　授課老師：<span class="text-primary"><?=$row['c_t_name']?></span><a href="?g=course"><button class="btn btn-sm btn-primary" style="float:right">月曆模式</button></a></div>
            <?php   }while($row = mysqli_fetch_assoc($ro));
                    }else{ ?>
                        <div>今日　<span class="text-primary"><?=$today?></span>　課程：<span class="text-danger"> 無 </span><a href="?g=course"><button class="btn btn-sm btn-primary" style="float:right">月曆模式</button></a></div>
            <?php   }?>
                ...
                <div>
                    ...
                    <nav id="nav" aria-label="Page navigation example">
                        <ul class="pagination justify-content-center">
                            <li class="page-item">
                            <a class="page-link" href="?g=class_search&page=<?=$p1?>">Previous</a>
                            </li>

                            <?php for($ii = 1; $ii <= $page_total ; $ii++) { 
                                if($ii == $page_now){
                                    ?>
                                    <li class="page-item"><a class="page-link  bg-primary text-white" href="?g=class_search&page=<?=$ii?>"><?=$ii?></a></li>


                            <?php }else{ ?>
                                    <li class="page-item"><a class="page-link" href="?g=class_search&page=<?=$ii?>"><?=$ii?></a></li>

                            <?php }
                                } ?>
                            
                            <li class="page-item">
                            <a class="page-link" href="?g=class_search&page=<?=$p2?>">Next</a>
                            </li>
                        </ul>
                    </nav>
                </div>
